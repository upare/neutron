...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight,
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    ...
    if (!txPrev.ReadFromDisk(txdb, txin.prevout, txindex))
    {
        ...
        return tx.DoS(1, error("%s : read txPrev failed", __func__));
    }

    // Verify signature
    if (!VerifySignature(txPrev, tx, 0, 0))
    {
        return tx.DoS(100, error("%s : VerifySignature failed on coinstake %s",
